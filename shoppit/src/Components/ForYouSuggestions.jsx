...
const ForYouSuggestions = ({ onAddToWishlist, wishlistProductIds = [] }) => {
  ...
  useEffect(() => {
    ...
    const loadSuggestions = async () => {
      ...
      try {
        const suggestions = await fetchForYouSuggestions(controller.signal)
        setProducts(suggestions)
      } catch (error) {
        if (error.name !== 'AbortError') {
          setProducts([])
          setErrorMessage('Could not load suggestions right now.')
        }
      } finally {
  ...
  const formattedProducts = useMemo(() => {
    ...
    return products.map((product) => ({
      ...
      typeLabel: product.category?.replace('-', ' ') || "men's clothing",
    }))
  }, [products])
  ...
  if (isLoading) {
    return (
      <section className='mt-6 rounded-2xl bg-slate-100 p-5'>
        <p className='text-sm font-medium text-slate-600'>Loading suggestions...</p>
      </section>
    )
  }
  ...
  return (
    <section className='mt-6 rounded-2xl bg-blue-100/70 p-4 sm:p-5'>
      <div className='mb-4 flex items-center justify-between'>
        <h2 className='text-xl font-bold text-slate-800'>Still looking for these?</h2>
        <button
